...
class ProfileView(APIView):
    def get(self, request):
        ...
        # Return a default error response if the method parameter is not recognized
        return Response({"status": "error", "code": 403, "message": "Invalid account name"})
...
class LikesView(APIView):
    def get(self, request):
        ...
        if method == "likes":
            ...
            response = requests.get(vk_post_url)
            data = response.json()
            ...
            # Fetch other data (shares, views) for the post using VK's wall.getById API
            ...
            return Response({
                ...
                "data": {
                    ...
                    "share": str(post_data.get("reposts", {}).get("count", "")),
                    "views": str(post_data.get("views", {}).get("count", ""))
                }
            })
